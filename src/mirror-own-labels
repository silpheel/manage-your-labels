...
query($owner:String!, $reposCursor:String, $labelsCursor:String) {
    organization(login:$owner) {
        repositories(first:100, after:$reposCursor, isArchived:false) {
...
if os.environ["GITHUB_EVENT_NAME"] not in {"schedule", "workflow_dispatch", "push"}:
    sys.exit(0)
...
try:
    current_repo = next(
        r
        for r in result['organization']['repositories']['nodes']
        if r['name'].lower() == '.github'
    )
    current_labels = current_repo['labels']['nodes']
except:
    print("Error reading this repo's labels")
    sys.exit(0)
